
import numpy as np
import pandas as pd
from dataclasses import dataclass

def _to_ts(x):
    if isinstance(x, pd.Timestamp): return x.normalize()
    if isinstance(x, str):          return pd.Timestamp(x)
    if isinstance(x, np.datetime64):return pd.Timestamp(x).normalize()
    raise TypeError("Expected pd.Timestamp or 'YYYY-MM-DD' string.")

def _doy(ts: pd.Timestamp) -> int:
    return int(ts.dayofyear)

@dataclass
class CanolaStageSampler:
    # Target MEAN shares (fractions of season). They will be normalised for the Dirichlet.
    mu_emerg_lag_rel: float = 0.05   # sow -> emergence
    mu_veg_lag_rel: float   = 0.45   # emergence -> end vegetative (SOF boundary)
    mu_anth_lag_rel: float  = 0.20   # end vegetative -> end anthesis (GF boundary)
    mu_grainfill_lag_rel: float = 0.25  # end anthesis -> end grainfill
    mu_mature_lag_rel: float = 10/109     # end grainfill -> maturity

    kappa: float = 40

    # How much of S->H the five phases should occupy (1.0 = whole season)
    total_share: float = 1.0

    # RNG seed
    seed: int | None = 123

    def __post_init__(self):
        self.rng = np.random.default_rng(self.seed)

        # Build the Dirichlet mean vector on the simplex
        mu = np.array([
            self.mu_emerg_lag_rel,
            self.mu_veg_lag_rel,
            self.mu_anth_lag_rel,
            self.mu_grainfill_lag_rel,
            self.mu_mature_lag_rel
        ], dtype=float)

        if mu.sum() <= 0:
            raise ValueError("All phase means are zero; please set positive means.")

        self.mu_simplex = mu / mu.sum()        # sums to 1
        self.alpha = self.mu_simplex * self.kappa  # Dirichlet parameters

        # Guard for total_share
        if not (0.0 < float(self.total_share) <= 1.0):
            raise ValueError("total_share must be in (0, 1].")

    def _sample_phase_shares(self) -> np.ndarray:
        """Sample five phase shares from Dirichlet and scale by total_share."""
        p = self.rng.dirichlet(self.alpha)     # sums to 1
        return self.total_share * p            # sums to total_share

    @staticmethod
    def _shares_to_days(total_days: int, shares: np.ndarray) -> np.ndarray:
        """
        Convert shares to integer days. If rounding overshoots, trim from the tail.
        Leaves any shortfall as 'remainder_days' automatically.
        """
        days = np.rint(shares * total_days).astype(int)
        overflow = int(days.sum() - total_days)
        i = len(days) - 1
        while overflow > 0 and i >= 0:
            take = min(overflow, days[i])
            days[i] -= take
            overflow -= take
            i -= 1
        return days  # [dE,dV,dA,dG,dM]

    def _sample_once(self, sowing, harvest):
        S = _to_ts(sowing)
        H = _to_ts(harvest)
        if H <= S:
            H = H + pd.DateOffset(years=1)

        total_days = (H - S).days
        if total_days < 5:
            raise ValueError("Sowing→harvest window too short.")

        shares = self._sample_phase_shares()
        dE, dV, dA, dG, dM = self._shares_to_days(total_days, shares)

        # Build stage boundaries
        Emergence = S + pd.Timedelta(days=int(dE))

        # Start of vegetative: immediately after emergence
        Veg_start = max(Emergence + pd.Timedelta(days=1), Emergence)

        # Start of flowering (SOF) at end of vegetative segment
        SOF = Emergence + pd.Timedelta(days=int(dV))
        SOF = max(SOF, Veg_start + pd.Timedelta(days=1))

        # Grainfill start at end of anthesis
        GF = SOF + pd.Timedelta(days=int(dA))
        GF = max(GF, SOF + pd.Timedelta(days=1))

        # Maturity start at end of grainfill
        Maturity = GF + pd.Timedelta(days=int(dG))
        Maturity = max(Maturity, GF + pd.Timedelta(days=1))

        # Keep maturity strictly before harvest (if rounding overshot)
        Maturity = min(Maturity, H - pd.Timedelta(days=1))
        if Maturity <= GF:
            Maturity = GF + pd.Timedelta(days=1)
            Maturity = min(Maturity, H - pd.Timedelta(days=1))

        remainder_days = max(0, (H - Maturity).days - int(dM))  # slack to harvest beyond the 'mature' segment

        return dict(
            sowing_ts=S, harvest_ts=H, emergence_ts=Emergence,
            vegetative_start_ts=Veg_start, flowering_start_ts=SOF,
            grainfill_start_ts=GF, maturity_start_ts=Maturity,

            sowing_doy=_doy(S), harvest_doy=_doy(H), emergence_doy=_doy(Emergence),
            vegetative_start_doy=_doy(Veg_start), flowering_start_doy=_doy(SOF),
            grainfill_start_doy=_doy(GF), maturity_start_doy=_doy(Maturity),

            # Expose lengths for inspection
            sow_to_em_days=int(dE),
            veg_days=int(dV),
            anthesis_days=int(dA),
            grainfill_days=int(dG),
            mature_lead_days=int(dM),        # intended lead to maturity within total_share
            remainder_days=int(remainder_days)
        )

    def sample(self, sowing, harvest, n: int = 1, as_dataframe: bool = True):
        rows = [self._sample_once(sowing, harvest) for _ in range(int(n))]
        if as_dataframe:
            return pd.DataFrame(rows)[[
                "sowing_ts","harvest_ts","emergence_ts",
                "vegetative_start_ts","flowering_start_ts","grainfill_start_ts","maturity_start_ts",
                "sowing_doy","harvest_doy","emergence_doy",
                "vegetative_start_doy","flowering_start_doy","grainfill_start_doy","maturity_start_doy",
                "sow_to_em_days","veg_days","anthesis_days","grainfill_days","mature_lead_days","remainder_days"
            ]]
        return rows

# Example:
# sampler = CanolaStageSampler(seed=42, kappa=40.0, total_share=1.0)
# df = sampler.sample("2025-05-01", "2025-09-05", n=5)
# print(df)

# import numpy as np
# import pandas as pd
# from dataclasses import dataclass

# def _to_ts(x):
#     """Convert various date-like inputs to a normalised pandas Timestamp (midnight)."""
#     if isinstance(x, pd.Timestamp):
#         return x.normalize()
#     if isinstance(x, str):
#         return pd.Timestamp(x)
#     if isinstance(x, np.datetime64):
#         return pd.Timestamp(x).normalize()
#     raise TypeError("Expected pd.Timestamp, np.datetime64 or 'YYYY-MM-DD' string.")

# def _doy(ts: pd.Timestamp) -> int:
#     """Day of year as int."""
#     return int(ts.dayofyear)


# @dataclass
# class CanolaStageSampler:
#     # Target MEAN shares (fractions of the phenology window).
#     # They will be renormalised onto the simplex for the Dirichlet.
#     # These correspond to 10, 48, 18, 23, 10 days out of 109 in the
#     # literature-based canonical canola development.
#     mu_emerg_lag_rel: float     = 10 / 109    # sow -> emergence
#     mu_veg_lag_rel: float       = 48 / 109   # emergence -> end vegetative (SOF)
#     mu_anth_lag_rel: float      = 18 / 109   # end vegetative -> end anthesis
#     mu_grainfill_lag_rel: float = 23 / 109   # end anthesis -> end grainfill
#     mu_mature_lag_rel: float    = 10 / 109   # end grainfill -> maturity

#     # Dirichlet concentration: larger => less variability around mu_simplex
#     kappa: float = 30.0

#     # How much of S->H the five phases should occupy (1.0 = whole S->H window).
#     # For long seasons, you may want total_share < 1 so phenology doesn't stretch.
#     total_share: float = 1.0

#     # RNG seed
#     seed: int | None = 123

#     # -------- Per-phase min/max durations in days (literature-based defaults) --------
#     emerg_min_days: int      = 4   # sow -> emergence
#     veg_min_days: int        = 45  # emergence -> SOF
#     anth_min_days: int       = 14  # SOF -> end anthesis
#     grainfill_min_days: int  = 14  # end anthesis -> end grainfill
#     mature_min_days: int     = 5   # end grainfill -> "maturity" lead

#     emerg_max_days: int | None      = 15
#     veg_max_days: int | None        = 50
#     anth_max_days: int | None       = 200
#     grainfill_max_days: int | None  = 200
#     mature_max_days: int | None     = 200
#     # -----------------------------------------------------------------------

#     def __post_init__(self):
#         self.rng = np.random.default_rng(self.seed)

#         # Build the Dirichlet mean vector on the simplex
#         mu = np.array([
#             self.mu_emerg_lag_rel,
#             self.mu_veg_lag_rel,
#             self.mu_anth_lag_rel,
#             self.mu_grainfill_lag_rel,
#             self.mu_mature_lag_rel
#         ], dtype=float)

#         if mu.sum() <= 0:
#             raise ValueError("All phase means are zero; please set positive means.")

#         self.mu_simplex = mu / mu.sum()           # sums to 1
#         self.alpha = self.mu_simplex * self.kappa # Dirichlet parameters

#         # Guard for total_share
#         if not (0.0 < float(self.total_share) <= 1.0):
#             raise ValueError("total_share must be in (0, 1].")

#         # Simple sanity checks for min/max
#         mins = np.array([
#             self.emerg_min_days,
#             self.veg_min_days,
#             self.anth_min_days,
#             self.grainfill_min_days,
#             self.mature_min_days
#         ], dtype=int)
#         if np.any(mins < 0):
#             raise ValueError("Minimum days must be non-negative.")

#         maxs = np.array([
#             self.emerg_max_days,
#             self.veg_max_days,
#             self.anth_max_days,
#             self.grainfill_max_days,
#             self.mature_max_days
#         ], dtype=object)

#         for min_val, max_val in zip(mins, maxs):
#             if max_val is not None and max_val < min_val:
#                 raise ValueError("Some max_days < min_days; please fix.")

#     def _sample_phase_shares(self) -> np.ndarray:
#         """Sample five phase shares from Dirichlet (summing to total_share)."""
#         p = self.rng.dirichlet(self.alpha)  # sums to 1
#         return self.total_share * p         # sums to total_share

#     def _shares_to_days(self, total_days: int, shares: np.ndarray) -> np.ndarray:
#         """
#         Convert shares to integer days with per-phase min/max constraints.

#         Guarantees:
#         - For each phase k: min_k <= d_k <= max_k (if max_k is not None).
#         - Sum(d_k) <= total_days.
#         """
#         total_days = int(total_days)

#         # Total days nominally allocated to the five phases,
#         # but cannot exceed the actual S->H window.
#         total_phase_days = int(round(self.total_share * total_days))
#         total_phase_days = min(total_phase_days, total_days)
#         if total_phase_days <= 0:
#             raise ValueError("total_share * total_days must be positive.")

#         mins = np.array([
#             self.emerg_min_days,
#             self.veg_min_days,
#             self.anth_min_days,
#             self.grainfill_min_days,
#             self.mature_min_days
#         ], dtype=int)

#         base_min = int(mins.sum())
#         if base_min > total_phase_days:
#             raise ValueError(
#                 f"Sum of minimum phase days ({base_min}) exceeds "
#                 f"total_phase_days ({total_phase_days}). "
#                 "Reduce minima or total_share, or increase the S->H window."
#             )

#         # Residual days to distribute on top of minima
#         residual_days = total_phase_days - base_min

#         if residual_days > 0:
#             # Normalise shares just in case total_share != 1
#             rel = shares / shares.sum()
#             extra = np.rint(rel * residual_days).astype(int)
#         else:
#             extra = np.zeros_like(mins, dtype=int)

#         days = mins + extra

#         # Fix rounding so that sum(days) <= total_phase_days,
#         # trimming only down to the per-phase minima.
#         overflow = int(days.sum() - total_phase_days)
#         if overflow > 0:
#             i = len(days) - 1
#             while overflow > 0 and i >= 0:
#                 room_to_cut = days[i] - mins[i]
#                 if room_to_cut > 0:
#                     take = min(overflow, room_to_cut)
#                     days[i] -= take
#                     overflow -= take
#                 i -= 1

#         # Apply per-phase max caps: any extra beyond max is just slack (not reallocated)
#         maxs = [
#             self.emerg_max_days,
#             self.veg_max_days,
#             self.anth_max_days,
#             self.grainfill_max_days,
#             self.mature_max_days
#         ]
#         for i, max_val in enumerate(maxs):
#             if max_val is not None and days[i] > max_val:
#                 days[i] = max_val

#         # Sanity checks: non-negative, respect minima, and sum <= total_days
#         if np.any(days < mins):
#             raise RuntimeError("Phase length fell below its minimum after allocation.")
#         if np.any(days < 0):
#             raise RuntimeError("Negative phase length after allocation.")
#         if days.sum() > total_days:
#             raise RuntimeError(
#                 f"Sum of phase days ({days.sum()}) exceeds total_days ({total_days}) "
#                 "even after applying min/max constraints. "
#                 "Check total_share, mins, and maxs."
#             )

#         return days  # [dE, dV, dA, dG, dM]

#     def _sample_once(self, sowing, harvest):
#         S = _to_ts(sowing)
#         H = _to_ts(harvest)
#         if H <= S:
#             H = H + pd.DateOffset(years=1)

#         total_days = (H - S).days
#         if total_days < 5:
#             raise ValueError("Sowing→harvest window too short.")

#         shares = self._sample_phase_shares()
#         dE, dV, dA, dG, dM = self._shares_to_days(total_days, shares)

#         # Build stage boundaries
#         Emergence = S + pd.Timedelta(days=int(dE))

#         # Start of vegetative: immediately after emergence
#         Veg_start = max(Emergence + pd.Timedelta(days=1), Emergence)

#         # Start of flowering (SOF) at end of vegetative segment
#         SOF = Emergence + pd.Timedelta(days=int(dV))
#         SOF = max(SOF, Veg_start + pd.Timedelta(days=1))

#         # Grainfill start at end of anthesis
#         GF = SOF + pd.Timedelta(days=int(dA))
#         GF = max(GF, SOF + pd.Timedelta(days=1))

#         # Maturity start at end of grainfill
#         Maturity = GF + pd.Timedelta(days=int(dG))
#         Maturity = max(Maturity, GF + pd.Timedelta(days=1))

#         # Keep maturity strictly before harvest (if rounding overshot)
#         Maturity = min(Maturity, H - pd.Timedelta(days=1))
#         if Maturity <= GF:
#             Maturity = GF + pd.Timedelta(days=1)
#             Maturity = min(Maturity, H - pd.Timedelta(days=1))

#         # slack to harvest beyond the 'mature' segment
#         remainder_days = max(0, (H - Maturity).days - int(dM))

#         return dict(
#             sowing_ts=S, harvest_ts=H, emergence_ts=Emergence,
#             vegetative_start_ts=Veg_start, flowering_start_ts=SOF,
#             grainfill_start_ts=GF, maturity_start_ts=Maturity,

#             sowing_doy=_doy(S), harvest_doy=_doy(H), emergence_doy=_doy(Emergence),
#             vegetative_start_doy=_doy(Veg_start), flowering_start_doy=_doy(SOF),
#             grainfill_start_doy=_doy(GF), maturity_start_doy=_doy(Maturity),

#             # Expose lengths for inspection
#             sow_to_em_days=int(dE),
#             veg_days=int(dV),
#             anthesis_days=int(dA),
#             grainfill_days=int(dG),
#             mature_lead_days=int(dM),
#             remainder_days=int(remainder_days)
#         )

#     def sample(self, sowing, harvest, n: int = 1, as_dataframe: bool = True):
#         rows = [self._sample_once(sowing, harvest) for _ in range(int(n))]
#         if as_dataframe:
#             return pd.DataFrame(rows)[[
#                 "sowing_ts","harvest_ts","emergence_ts",
#                 "vegetative_start_ts","flowering_start_ts","grainfill_start_ts","maturity_start_ts",
#                 "sowing_doy","harvest_doy","emergence_doy",
#                 "vegetative_start_doy","flowering_start_doy","grainfill_start_doy","maturity_start_doy",
#                 "sow_to_em_days","veg_days","anthesis_days","grainfill_days","mature_lead_days","remainder_days"
#             ]]
#         return rows


