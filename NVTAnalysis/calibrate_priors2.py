...
def objective_function(
    priors: np.ndarray,
    query: Query,
    daesim_config: DAESIMConfig,
    parameters: Parameters
)->float:
    
    loss = calibrate_dev(priors, query, 0, daesim_config, parameters).fun
    print(query.stub, loss, priors)
    return loss
    

def calibrate_priors2(
        query: Query,
    ...
    result = differential_evolution(
        objective_function,
        bounds = bounds,
        args=(query, daesim_config, parameters),
        popsize=1,
        tol=0.2,
        maxiter=10,
        workers=1,
...
def test():
    ...
    calibrate_priors2(queries[3], experiment_df)
